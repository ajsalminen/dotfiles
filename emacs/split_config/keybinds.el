...
(defvar my-keys-minor-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "M-p") 'jump-char-forward)
    (define-key map (kbd "M-f") 'jump-char-backward)
    (define-key map (kbd "C-E") 'backward-kill-sexp)
    (define-key map (kbd "C-S-R") 'kill-sexp)
    (define-key map (kbd "C-S-L") 'forward-sexp)
    (define-key map (kbd "C-S-J") 'backward-sexp)
